...
try:
    replace_once(
        ...
        const probed = mapped?.kind === "record" ? null : probeLower(L, arg);
        const recT = mapped?.kind === "record" ? mapped : probed?.type.kind === "record" ? probed.type : null;
        if (arg.getSourceFile().fileName.includes("/core/extensions/runner.ts")) {
          ...
          console.error(
            `[SCRIPTC_RUNNER_ENTRIES_PROBE] file=${arg.getSourceFile().fileName} text=${arg.getText()} ts=${L.checker.typeToString(argTsType)} flags=${argTsType.flags} indexes=${indexText || "none"} mapped=${mapped ? L.fmt(mapped) : "null"} probed=${probed ? L.fmt(probed.type) : "null"}`,
          );
        }
        if (recT) {''',
    )
    ...
    prime = '/tmp/scriptc-prime-runner-entries-probe'
    ...
    code, text = run(['node', cli, 'build', 'packages/coding-agent/src/cli-main.ts', '--dynamic', '-o', '/tmp/prime-runner-probe-bin'], cwd=prime, env=env)
    probe = [line for line in text.splitlines() if 'SCRIPTC_RUNNER_ENTRIES_PROBE' in line]
    relevant = [line for line in text.splitlines() if 'runner.ts:84' in line or 'Object.entries' in line]
    LOG.append(f'prime-scriptc-exit={code}')
    LOG.append('=== runner probe lines ===')
    LOG.extend(probe)
    LOG.append('=== Object.entries diagnostics ===')
    LOG.extend(relevant[-80:])
    if not probe:
        LOG.append('NO RUNNER PROBE LINES EMITTED')
