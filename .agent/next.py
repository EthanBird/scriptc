...
FAIL = Path('OBJECT_ENTRIES_SNAPSHOT_FAILURE.txt')
LOG: list[str] = []
...
def replace_once(old: str, new: str) -> None:
    ...
    if count != 1:
        raise RuntimeError(f'expected one anchor, found {count}: {old[:110]!r}')
...
try:
    ...
    if 'snapshot-source-after' not in text:
...
if ok:
    pattern = '3003-object-entries-forof|object-entries'
...
if ok:
    if FAIL.exists():
        FAIL.unlink()
    print('OBJECT_ENTRIES_SNAPSHOT=PASS', flush=True)
else:
    subprocess.run(['git', 'checkout', '--', str(SOURCE), str(CORPUS)], check=False)
    LOG.append('OBJECT_ENTRIES_SNAPSHOT=FAIL; source/corpus reverted')
    FAIL.write_text('\n'.join(LOG) + '\n', encoding='utf-8')
    print('OBJECT_ENTRIES_SNAPSHOT=FAIL', flush=True)
