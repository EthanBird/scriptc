...
try:
    ...
    repl2='''          if (looseJsDeadMethodAliasField(L, decl, member)) continue;\n          const type = L.irTypeOf(member.name);\n          if (type.kind === "void") L.badType(member.name, L.typeOf(member.name));\n'''
    text=rep(text,anchor2,repl2,'field collection insertion')
...
# Focused executable probes through the real CLI. The first field is dead and\n# should compile only in loose-js; the second is read and must keep the fence.
if ok:
    tmp=Path('/tmp/methodval-worker'); tmp.mkdir(exist_ok=True)
    dead=tmp/'dead.js'; dead.write_text('''class M {\n  options = this.setOptions;\n  value = 1;\n  setOptions(o) { this.value = o; return this; }\n}\nconst m = new M();\nconsole.log(m.value);\n''')
    live=tmp/'live.js'; live.write_text('''class M {\n  options = this.setOptions;\n  value = 1;\n  setOptions(o) { this.value = o; return this; }\n}\nconst m = new M();\nconsole.log(typeof m.options);\n''')
    cli=str(Path.cwd()/'packages/cli/dist/main.js')
    rc,out=run(['node',cli,'build',str(dead),'--dynamic','--loose-js','-o',str(tmp/'dead-bin')],env=ENV,timeout=900)
    if rc!=0: ok=False
    if rc==0:
        rr,ro=run([str(tmp/'dead-bin')],env=ENV,timeout=60)
        if rr!=0 or ro.strip()!='1': ok=False; log.append('dead-field runtime mismatch')
    rc_live,out_live=run(['node',cli,'build',str(live),'--dynamic','--loose-js','-o',str(tmp/'live-bin')],env=ENV,timeout=900)
    if rc_live==0 or 'bound method references' not in out_live:
        ok=False; log.append('live field unexpectedly lost the method-value fence')
prime=[]
if ok:
