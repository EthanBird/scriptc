...
FILES = {
    ...
    'c_exprs': Path('packages/compiler/src/backend/emission/emit-exprs.ts'),
...
def run(args: list[str], env: dict[str, str]) -> bool:
    ...
    try:
        ...
        LOG.extend((cp.stdout or '').splitlines()[-340:])
...
ok = True
try:
    replace_once(FILES['nodes'], '  | "fsp.writeFile"\n  | "fsp.mkdir"', '  | "fsp.writeFile"\n  | "fsp.access"\n  | "fsp.mkdir"')
    replace_once(
        FILES['surfaces'],
    ...
    replace_once(FILES['fallback'], '  export function writeFile(path: string, data: string): Promise<void>;\n', '  export function access(path: string, mode?: number): Promise<void>;\n  export function writeFile(path: string, data: string): Promise<void>;\n')
    replace_once(FILES['runtime_h'], 'ScrPromise *scr_fsp_write_file(ScrStr *path, ScrStr *data);\nScrPromise *scr_fsp_mkdir', 'ScrPromise *scr_fsp_write_file(ScrStr *path, ScrStr *data);\nScrPromise *scr_fsp_access(ScrStr *path, double mode);\nScrPromise *scr_fsp_mkdir')
    replace_once(
        FILES['runtime_c'],
...
ScrPromise *scr_fsp_mkdir''',
    ...
    replace_once(FILES['llvm'], '  "fsp.writeFile": "scr_fsp_write_file",\n  "fsp.mkdir":', '  "fsp.writeFile": "scr_fsp_write_file",\n  "fsp.access": "scr_fsp_access",\n  "fsp.mkdir":')
    replace_once(
        FILES['c_exprs'],
        '''          case "fsp.writeFile":
...
if ok:
    ...
    for path in FILES.values(): subprocess.run(['git', 'checkout', '--', str(path)], check=False)
