...
SRC=Path('packages/compiler/src/frontend/lowering/lower-classes.ts'); orig=SRC.read_text(); REPORT=Path('WORKER_TEXTDECODER_CONTEXT.txt')
...
try:
 ...
 p=Path('/tmp/prime-textdec-context'); shutil.rmtree(p,ignore_errors=True)
 ...
 rc,out=run(['node',cli,'build','packages/coding-agent/dist/bundle/cli.js','--dynamic','--loose-js','-o','/tmp/prime-textdec-bin'],cwd=p,timeout=1200)
 m=re.search(r'([^\n:]+\.js):(\d+):(\d+) - error SC2020: \'new TextDecoder\'',out)
 if not m: raise RuntimeError('TextDecoder blocker not found\n'+out[-12000:])
 f=Path(m.group(1)); center=int(m.group(2)); lines=f.read_text(encoding='utf-8').splitlines(); a=max(1,center-80); b=min(len(lines),center+100)
 report=[f'build_status={rc}',f'file={f}',f'line={center}',f'\n=== context {a}-{b} ===']+[f'{i:06d}: {lines[i-1]}' for i in range(a,b+1)]
 REPORT.write_text('\n'.join(report)+'\n')
except Exception as e:
 REPORT.write_text('TEXTDECODER_CONTEXT=FAIL\n'+repr(e)+'\n'+traceback.format_exc())
finally:
 SRC.write_text(orig)
