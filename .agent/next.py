...
if not ok:
    ...
    FAIL.write_text(chr(10).join(LOG)+chr(10),encoding='utf-8')
else:
    ...
    report=['head_task=JS Error(any) coercion + Math.random validation',f'build_status={build_status}',f'smoke_status={smoke_status}',f'diagnostic_count={len(diags)}','--- diagnostics ---']
    report.extend(diags)
    report.append('--- smoke tail ---')
    report.extend(smoke_out.splitlines()[-120:])
    PRIME.write_text(chr(10).join(report)+chr(10),encoding='utf-8')
