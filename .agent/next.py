...
try:
 ...
 n=orig.count(old)
 if n!=1: raise RuntimeError(f'Set anchor count={n}')
...
if ok:
 ...
 # strict loose-js-off build keeps the historical Set<any> diagnostic
 rc2,out2=run(['node',cli,'build',str(f),'--dynamic','-o',str(root/'strict-bin')],env=ENV,timeout=900)
 if rc2==0 or 'Set elements of type' not in out2:
  ok=False; log.append('strict lane unexpectedly accepted Set<any>')
prime=[]
if ok:
